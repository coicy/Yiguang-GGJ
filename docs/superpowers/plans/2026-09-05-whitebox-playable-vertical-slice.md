...
- Use `A/D`, `Space`, `Q`, `F`, `R`, and `F3` as specified; normal gameplay must not use manual form cycling.
...
- `project.godot`: add `ability_secondary` mapped to `F`; preserve existing actions.
...
- Produces: `AbilityController.setup(player, movement, forms, leg_area)`, `set_primary_pressed(pressed)`, `set_secondary_pressed(pressed)`, `try_root()`, `try_extend_legs()`, `stop_primary()`, `stop_secondary()`, `cancel_all()`, `is_rooted()`, `is_leg_extended()`, `is_vine_attached()`.
...
- [ ] **Step 1: Add `ability_secondary` and write failing player ability tests**
...
abilities.set_primary_pressed(Input.is_action_pressed("ability_primary"))
abilities.set_secondary_pressed(Input.is_action_pressed("ability_secondary"))
...
Launch `Godot_v4.7.2-stable_win64.exe --path <repo>`. Use only `A/D`, `Space`, `Q`, `F`, and optional `R`. Verify, in order: sprout tunnel, nutrition growth to humanoid, root against active wind, leg switch, checkpoint, growth to mature, vine attach/release, glide landing, toxin wither to sprout, final low switch, unlocked exit, completion overlay. Do not invoke debug form switching or scene methods.
